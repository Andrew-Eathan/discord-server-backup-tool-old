...
process_continue = _ => {
    ...
    // get channels
    mapping = [];
    selectedGuild.channels.cache.forEach(value => {
        if (value.type == "text") mapping.push(value);
    })
...
process_begin = async _ => {
    ...
    if (chosen_options.save_server_data) {
        ...
        // save icon
        svinfo.file("icon.png", getLinkData(guild.iconURL() ?? "https://cdn.discordapp.com/attachments/947806697878069249/953378638152228874/unknown.png?size=256"))
        svinfo.file("banner.webp", getLinkData(guild.bannerURL() ?? "https://cdn.discordapp.com/attachments/868443505045962783/953379661444948078/unknown.png?size=256"))
    ...
    if (chosen_options.save_members) {
        ...
        let str = `List of ${members.length} members:\r\n\r\n`
        ...
        for (var member of members) {
            ...
            //whoops, was on v13 docs and forgot i use 12.5.3 - let timeout = member.isCommunicationDisabled() ? `Yes, until ${dateAndTime(ttime, chosen_options.time_locale)}` : "No"
            ...
            str = str
                ...
                + `Activities: Currently ${member.presence.activities.length}\r\n`
    ...
    if (chosen_options.save_roles) {
        ...
        let str = `List of ${roles.cache.size} roles:\r\n\r\n`
        let role_idx = 0;

        for (var role of roles.cache) {
            role_idx++;
            role = role[1]
    ...
    ban_save: if (chosen_options.save_bans) {
        ...
        try {
            ...
            console.log("Couldn't save bans, you probably don't have access to them!")
            console.log(e)
            break ban_save;
        }
                ...
                for (var msg of messages) {
                    ...
                    switch (msg?.type) {
                        case "DEFAULT":
                            ...
                            else // probably a sticker, can't read them in v12.5.3
                                final_text = `${time_string} ${msg.author.tag}${msg_reply} - sent sticker (i think?)\r\n`
                        break;
                        case "RECIPIENT_ADD":
    ...
    try {
        fs.writeFileSync(guild.name + ".zip", content)
        console.log("Saved to " + guild.name + ".zip!")
    }
    catch (e) {
